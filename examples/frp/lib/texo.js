...
(function (factory) {
	...
	function replace () {
		...
		listFunc._depth = parent._depth + 1;
		listFunc.count = Math.max(parent.count, replacements.length);
		return addMethods(listFunc);
	}
	...
	function inifiteCheck(items) {
		if (items.count === Infinity) {
			throw Error("Attempted to traverse an infinite list");
		}
	}
	...
	function addMethods (listFunc) {
		...
		if (count > 1 && listFunc._depth > Math.log2(count) * 10) {
