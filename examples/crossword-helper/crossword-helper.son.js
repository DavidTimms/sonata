(function () {
    ...
    fs = require('fs');
    function main(query) {
        return fs.readFileSync('word-list.txt', 'utf8').split('\n').map(function (word) {
            return word.trim();
        }).filter(matches(query.toUpperCase())).forEach(function (word) {
            return print(word);
        });
    }
    function matches(query) {
        ...
        return function (word) {
            return eq(query.length, word.length) && queryChars.every(function (char, i) {
                return eq(char, '_') || eq(char, word.charAt(i));
            });
        };
    }
    $sonata_startMain();
